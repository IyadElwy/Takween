        ...
        match processing_type:
            case 'diacriticsRemoval':
                ...
                diacriticsRemoval.process_diacritics_removal(
                    job_data, file_path, original_datasource_path.location, field_to_process
                )  # type: ignore
            ...
            case 'flattening':
                original_datasource_path = await FileDataSource.filter(id=job_data['dataSourceId']).first()
                flattening.process_flattening(job_data, file_path, original_datasource_path.location)
                file_location = convert_ndjson_to_json_and_save(file_path)
                created_file_data_source = await FileDataSource.create(
                    file_name=f'{data_name}',
                    file_type=parse_file_type_enum('json'),
            ...
            case 'merging':
                ...
                for ds_id in job_data['dataSourceIds']:
                    ...
                    original_datasource_paths.append(curr_data_source.location)  # type: ignore
                field_to_process = job_data['fieldToProcess']
                merging.process_merging(job_data, file_path, original_datasource_paths, field_to_process)
                file_location = convert_ndjson_to_json_and_save(file_path)
                created_file_data_source = await FileDataSource.create(
                    file_name=f'{data_name}',
                    file_type=parse_file_type_enum('json'),
            ...
            case 'tokenizing':
                original_datasource_path = await FileDataSource.filter(id=job_data['dataSourceId']).first()
                field_to_process = job_data['fieldToProcess']
                tokenizing.process_tokenizing(job_data, file_path, original_datasource_path.location, field_to_process)  # type: ignore
                file_location = convert_ndjson_to_json_and_save(file_path)
                created_file_data_source = await FileDataSource.create(
                    file_name=f'{data_name}',
                    file_type=parse_file_type_enum('json'),
...
async def ocr(
    ...
    try:
        ...
        for file in files:
            ...
            result_file_location = f'data/{projectId}-{uuid.uuid4()}-{file.filename}'.replace(file_type, 'json')
            ...
            with open(result_file_location, 'w+') as f:
                ...
                final_json = []
                for t in result:
                    final_json.append({'text': t})
                f.write(json.dumps(final_json))
            created_file_data_source = await FileDataSource.create(
                file_name=job_data['title'],
                file_type='json',
                location=result_file_location,
                size=os.path.getsize(result_file_location),
                project=project,
                created_by=user,
            )
