...
async def get_project_jobs(id, request: Request):
    try:
        ...
        jobs = await project.get_jobs(
            Q(assigned_annotators=user)  # type: ignore
            | Q(created_by=user)
        )
        ...
        match annotation_type:
            case 'textClassification':
                ...
                created_job = await TextClassificationJob.create(
                    ...
                    active_learning=True if job_data.get('active_learning') else False,
                    created_by=user,
                )
            ...
            case 'partOfSpeech':
                ...
                created_job = await PartOfSpeechJob.create(
                    ...
                    field_to_annotate=job_data['fieldToAnnotate'],
                    active_learning=True if job_data.get('active_learning') else False,
                    tags_list_as_string=str(job_data['tags']),
                    created_by=user,
                )
                await created_job.assigned_annotators.add(user)
                return created_job
