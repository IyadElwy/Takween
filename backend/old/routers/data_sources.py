    ...
    try:
        ...
        for data_source in file_data_sources_list:
            with open(data_source['location'], 'r') as json_file:
                ...
                data_source['exampleData'] = get_json_sample_from_file(json_data, depth=5, max_elements=1)
...
async def add_project_data(projectId, request: Request, files: list[UploadFile] = Form(...)):
    try:
        ...
        for file in files:
            ...
            if file_type == 'csv':
                file_location = convert_csv_to_json_and_save(file_location)
            elif file_type == 'tsv':
                file_location = convert_tsv_to_json_and_save(file_location)
                file_type = 'json'
            ...
            created_file_data_source = await FileDataSource.create(
                file_name=file_name,
                file_type=parse_file_type_enum(file_type),  # type: ignore
        ...
        with open(file_data_source['location'], 'r') as json_file:
            json_data = json.load(json_file)
            file_data_source['exampleData'] = get_json_sample_from_file(json_data, depth=5, max_elements=1)

        return {'created_file_data_sources': file_data_source}
